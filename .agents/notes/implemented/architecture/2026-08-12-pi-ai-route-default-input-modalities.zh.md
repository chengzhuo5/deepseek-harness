...
**没有任何配置界面编辑 `input`。** 它和 `compat`、`thinkingBudgets`、`headers` 一样是 settings 文档字段。`ModelListEditor` 通过思考档位复选列表编辑 `reasoningEfforts`；网关特有的协议拼写仍留在 `settings.yaml`。它的行 patch 依然会先展开已存储的行再应用改动，而采纳候选时已有行优先于重新发现的候选，因此手写的 `input` 在两条路径上都能存活。
