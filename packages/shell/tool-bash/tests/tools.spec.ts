...
describe('sandbox escalation through the generic task producer', () => {
  ...
  it('rejects injected escalation without a sandbox and an unknown escalation without prompting', async () => {
    ...
    const result = await call(ctx, 'bash', { ...escalate, sandbox_permissions: 'unknown-mode' }, sandboxAgent('workspace-write'))
