...
export const en = {
  ...
  modelThinkLevels: 'Thinking levels',
  modelThinkHint: 'Tick the thinking levels this model offers; unticked levels are not selectable. Leave them all unticked to keep the provider default (a custom model then does not think).',
  modelReasoningInvalid: 'Thinking levels are invalid: offer at least one level beyond "off", each named with its wire value.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  modelThinkLevels: '思考档位',
  modelThinkHint: '勾选该模型支持的思考档位；未勾选的档位在模型选择器中不会出现。全部不勾选表示沿用提供方默认（自定义模型将不支持思考）。',
  modelReasoningInvalid: '思考档位配置无效：请至少提供一个非 off 的档位，并为每个档位写入线上取值。',
