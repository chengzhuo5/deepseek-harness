 ...
 * background. The edge tracker is deliberately the same pattern the Host's
 * completed-reminder projection uses (first observation records the bit and
...
export class SessionCompletionWatcher {
   ...
   * @param body - localized completion copy.
   * @param open - navigation action for an activated notification.
   */
  constructor(
  ...
  private raise(row: SessionSummary): void {
    ...
    this.sink.show(this.title(row), this.body, () => { this.open(row.id) })
  }
}
